(() => {
  ...
  function removeDevForNonDev(){if(isDev())return;$$('[data-cgtab="dev"]').forEach(x=>x.remove());const v=$('#cg-dev');if(v){v.classList.remove('active');v.innerHTML=''}if(S.profile&&S.profile.tier!=='DEV'&&document.querySelector('.cg-tab.active')?.dataset.cgtab==='dev')document.querySelector('[data-cgtab="packs"]')?.click()}

  function patchCardMakerTarget(){if(!isDev()||!S.boot)return;const sel=$('#cg-dev-target');if(!sel)return;const old=sel.value;sel.innerHTML=clientOptions(old||S.boot.self_key_id);if(!sel.value&&S.boot.self_key_id)sel.value=S.boot.self_key_id;if(!sel.parentElement?.classList.contains('dg-target-wrap')){const w=document.createElement('div');w.className='dg-target-wrap';sel.parentNode.insertBefore(w,sel);w.innerHTML='<div class="dg-label">Target client</div>';w.appendChild(sel)}}
  ...
  function patchDev(){const dev=$('#cg-dev');if(!dev)return;if(!isDev()){removeDevForNonDev();return}if(!S.boot)return;const grid=$('.cg-dev-grid',dev);if(!grid)return;let first=grid.firstElementChild;if(!first){first=document.createElement('div');first.className='cg-panel';grid.prepend(first)}if(first.dataset.dgPackGiver!=='1'){first.dataset.dgPackGiver='1';first.innerHTML=packGiverMarkup();$('#dg-grant',first).onclick=grantPack}else if(!$('#dg-target',first)){first.innerHTML=packGiverMarkup();$('#dg-grant',first).onclick=grantPack}patchCardMakerTarget()}
  ...
  function patchGiftPreviews(){for(const pack of S.gifts||[]){if(pack.status!=='READY')continue;const btn=$(`[data-openpack="${pack.id}"]`);const box=btn?.closest('.cg-pack');if(!box)continue;let p=$('.dg-preview',box);if(!p){p=document.createElement('div');p.className='dg-preview';box.insertBefore(p,btn)}p.innerHTML=`<div class="dg-preview-title">Known gift contents</div>${(pack.entries||[]).map(e=>`<div class="dg-preview-row"><span>${Number(e.slot)}</span><b>${esc(S.names.get(Number(e.player_user_id))||`Roblox ${Number(e.player_user_id)}`)}</b><span class="dg-rarity">${esc(e.rarity)}</span></div>`).join('')}`}}
